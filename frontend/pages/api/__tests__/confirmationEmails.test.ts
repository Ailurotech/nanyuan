...
const mockRequestResponse = (method: string, body: any) => {
  const req = { method, body, url: '/api/confirmationEmails' } as unknown as NextApiRequest;
  const res = {
...
describe("ConfirmationEmails API", () => {
  ...
  it("should return 400 if type is missing", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: "Missing required field: type" });
  });
  ...
  it("should successfully send confirmation email for TakeAwayOrder", async () => {
    ...
    const { req, res } = mockRequestResponse("POST", { type: "TakeAwayOrder", orderId });
    await handler(req, res);
  ...
  it("should successfully send confirmation email for Reservation", async () => {
    ...
    const { req, res } = mockRequestResponse("POST", {...reservationInfo, type: "Reservation"});
    await handler(req, res);
  ...
  it("should return 400 if email content is missing even with a valid type (simulated)", async () => {
    ...
    const { req, res } = mockRequestResponse("POST", { type: "ReservationX" as "Reservation" });
    await handler(req, res);
    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({ error: "Email content is missing." });
  });
